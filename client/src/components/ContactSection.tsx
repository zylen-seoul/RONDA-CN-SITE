...
import { localizedPath, useLanguage, type Language } from '@/contexts/LanguageContext';
...
const CITY_MEDIA: Record<
  Language,
  {
    globalLink: string;
    hangzhou: { tag: string; title: string; body: string; alt: string };
    seoul: { tag: string; title: string; body: string; alt: string };
  }
> = {
  zh: {
    globalLink: '了解杭州与首尔协作',
    hangzhou: {
      tag: 'SUPPLY CHAIN',
      title: '杭州 · 供应链协作中心',
      body: '统筹产品标准、资源匹配、生产节点、质量检查与项目交付。',
      alt: '杭州城市景观',
    },
    seoul: {
      tag: 'DESIGN & MARKET',
      title: '首尔 · 协作触点',
      body: '承接首尔当地的设计与市场沟通，为跨市场项目提供协同支持。',
      alt: '首尔城市景观',
    },
  },
  en: {
    globalLink: 'Explore Hangzhou and Seoul coordination',
    hangzhou: {
      tag: 'SUPPLY CHAIN',
      title: 'Hangzhou · Supply Coordination',
      body: 'Coordinates product standards, resource matching, production milestones, quality checks and delivery.',
      alt: 'Hangzhou city view',
    },
    seoul: {
      tag: 'DESIGN & MARKET',
      title: 'Seoul · Collaboration Touchpoint',
      body: 'Supports local design and market communication in Seoul for cross-market projects.',
      alt: 'Seoul city view',
    },
  },
  ko: {
    globalLink: '항저우와 서울 협업 보기',
    hangzhou: {
      tag: 'SUPPLY CHAIN',
      title: '항저우 · 공급망 협업 중심',
      body: '제품 기준, 자원 매칭, 생산 단계, 품질 검사와 프로젝트 납품을 조율합니다.',
      alt: '항저우 도시 전경',
    },
    seoul: {
      tag: 'DESIGN & MARKET',
      title: '서울 · 협업 접점',
      body: '서울 현지의 디자인과 시장 커뮤니케이션을 연결해 다양한 시장의 프로젝트를 지원합니다.',
      alt: '서울 도시 전경',
    },
  },
};
  ...
  return (
    <section id="contact" data-lang={lang} className={`contact-section py-10 md:py-16${showCityMedia ? ' is-home' : ''}`}>
      <div className="container contact-section__container">
        {/* Section header */}
        <motion.div ref={ref} initial={{ opacity: 0, y: 24 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="contact-section__header flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-8">
          <div>
            <span
              style={{
                display: 'block',
            ...
            >
              {showCityMedia ? t('contact.title').replace(/\n/g, ' ') : t('contact.title')}
            </h2>
          </div>
          <p
            style={{
        ...
        {/* Hangzhou & Seoul Info */}
        <div className="contact-section__city-grid grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          {/* Hangzhou */}
          <motion.div className="contact-section__city-card" initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay: 0.1 }}>
            {showCityMedia && (
              <div className="contact-section__city-media">
                <img src={HANGZHOU_IMAGE} alt={cityMedia.hangzhou.alt} loading="lazy" decoding="async" />
                <div className="contact-section__city-caption">
                  <span>{cityMedia.hangzhou.tag}</span>
                  <h3>{cityMedia.hangzhou.title}</h3>
                  <p>{cityMedia.hangzhou.body}</p>
                </div>
              </div>
            )}
            <div className="contact-section__city-details">
          ...
          {/* Seoul */}
          <motion.div className="contact-section__city-card" initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay: 0.2 }}>
            {showCityMedia && (
              <div className="contact-section__city-media is-seoul">
                <img src={SEOUL_IMAGE} alt={cityMedia.seoul.alt} loading="lazy" decoding="async" />
                <div className="contact-section__city-caption">
                  <span>{cityMedia.seoul.tag}</span>
                  <h3>{cityMedia.seoul.title}</h3>
                  <p>{cityMedia.seoul.body}</p>
                </div>
              </div>
            )}
            <div className="contact-section__city-details">
        ...
        {showCityMedia && (
          <a className="contact-section__global-link" href={localizedPath('/global/', lang)}>
            {cityMedia.globalLink}
      ...
      {/* QR Modal */}
      <AnimatePresence>{activeQR && <QRModal name={activeQR.name} qr={activeQR.qr} onClose={() => setActiveQR(null)} />}</AnimatePresence>
    </section>
  );
}
