...
type SampleAppSectionProps = {
  embedded?: boolean;
};

export default function SampleAppSection({ embedded = false }: SampleAppSectionProps) {
  ...
  return (
    <section
      id="sampleapp"
      aria-label={embedded ? t('sampleapp.title') : undefined}
      className={`sampleapp-module${embedded ? ' is-embedded' : ''}`}
      style={{
        background: embedded ? 'transparent' : 'linear-gradient(160deg, #EAE6DF 0%, #DDD8D0 50%, #E8E4DC 100%)',
        padding: embedded ? 'clamp(1.25rem, 2.5vw, 2rem) 0 clamp(2.5rem, 5vw, 4rem)' : 'clamp(2.5rem, 5vw, 4rem) 0',
        position: 'relative',
        overflow: 'hidden',
      }}
    >
      ...
      >
        ...
        {!embedded && (
          <div
            className="sampleapp-module__heading"
            style={{
              textAlign: 'center',
        ...
        {/* ── 三大核心优势图标（紧凑横排） ── */}
        <div
          className="sampleapp-module__features"
          style={{
            display: 'grid',
        ...
        {/* ── 主体：左侧导航 + 中间手机图 + 右侧文字 ── */}
        <div
          className="sampleapp-module__body"
          style={{
            display: 'grid',
        ...
        >
          ...
          <div className="sampleapp-module__nav" style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
            {SLIDE_KEYS.map((sk_key, idx) => (
              <button
                key={idx}
                aria-label={t(`sampleapp.${sk_key}.title`)}
                ...
                style={{
                  ...
                  textAlign: 'left',
                  position: 'relative',
                  borderLeft: activeSlide === idx ? '2px solid #b8956a' : '2px solid rgba(184,149,106,0.2)',
          ...
          <div
            className="sampleapp-module__phone"
            style={{
              display: 'flex',
          ...
          {/* 右侧：当前幻灯片文字 */}
          <div
            className="sampleapp-module__copy"
            style={{
        ...
        >
          {SLIDE_KEYS.map((_, idx) => (
            <button
              key={idx}
              aria-label={t(`sampleapp.${SLIDE_KEYS[idx]}.title`)}
              aria-pressed={activeSlide === idx}
              onClick={() => resetAutoPlay(idx)}
              style={{
        ...
        >
          ...
          {/* 三列并列 */}
          <div
            className="sampleapp-module__ai-grid"
            style={{
              display: 'grid',
              gridTemplateColumns: 'repeat(3, 1fr)',
